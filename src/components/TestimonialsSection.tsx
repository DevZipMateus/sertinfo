...
const TestimonialsSection = () => {
    ...
    const preloadImages = () => {
      testimonials.forEach(testimonial => {
        ...
        img.onerror = () => setImagesLoaded(prev => ({
          ...prev,
          [testimonial.image]: true
        })); // Consider it loaded even if error
      });
    };
    preloadImages();
  }, []);
  ...
  return <section id="testimonials" ref={sectionRef} className="section-padding bg-gray-50 overflow-hidden">
      <div className="container mx-auto">
        <div className="text-center mb-16">
          <span className="inline-block px-4 py-1 bg-primary/10 text-primary rounded-full text-sm font-medium mb-4 animate-on-scroll">
            Depoimentos
          </span>
          <h2 className="text-3xl md:text-4xl font-bold mb-4 animate-on-scroll"> Veja o que nossos clientes dizem:</h2>
        ...
        <div className="relative max-w-5xl mx-auto px-4">
          <div className="animate-on-scroll">
            <div className={`grid grid-cols-1 md:grid-cols-2 gap-6 testimonial-slider transform`}>
              {getVisibleTestimonials().map((testimonial, index) => <TestimonialCard key={index} testimonial={testimonial} isLoaded={Boolean(imagesLoaded[testimonial.image])} />)}
            </div>
          </div>

          <div className="flex justify-center mt-10 space-x-4">
            <button onClick={handlePrev} className="p-3 rounded-full bg-white shadow-md border border-gray-200 text-foreground hover:bg-gray-50 transition-colors" aria-label="Depoimento anterior">
              <ChevronLeft size={20} />
            </button>
            ...
            <div className="flex space-x-2">
              ...
              setTimeout(() => setIsTransitioning(false), 600);
            }} className={`w-3 h-3 rounded-full transition-colors ${currentIndex === index || index === (currentIndex + 1) % testimonials.length && window.innerWidth >= 768 ? 'bg-primary' : 'bg-gray-300'}`} aria-label={`Ir para depoimento ${index + 1}`} />)}
            </div>
            
            <button onClick={handleNext} className="p-3 rounded-full bg-white shadow-md border border-gray-200 text-foreground hover:bg-gray-50 transition-colors" aria-label="Próximo depoimento">
...
const TestimonialCard = ({
  testimonial,
  isLoaded
}: TestimonialCardProps) => <Card className="border border-border/50 shadow-md service-card h-full">
    <CardContent className="p-8 h-full flex flex-col">
      ...
      <div className="flex items-center">
        {!isLoaded ? <Skeleton className="w-12 h-12 rounded-full mr-4" /> : <Avatar className="h-12 w-12 mr-4">
